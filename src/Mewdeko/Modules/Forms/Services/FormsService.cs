...
public class FormsService : INService
{
    ...
    {
        await using var db = await dbFactory.CreateConnectionAsync();
        var updated = await db.FormQuestionOptions
            .Where(o => o.Id == option.Id)
            .Set(o => o.OptionText, option.OptionText)
            .Set(o => o.OptionValue, option.OptionValue)
            .Set(o => o.DisplayOrder, option.DisplayOrder)
            .UpdateAsync();
        return updated > 0;
    }
