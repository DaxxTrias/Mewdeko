// ---------------------------------------------------------------------------------------------------
// <auto-generated>
// This code was generated by LinqToDB scaffolding tool (https://github.com/linq2db/linq2db).
// Changes to this file may cause incorrect behavior and will be lost if the code is regenerated.
// </auto-generated>
// ---------------------------------------------------------------------------------------------------

using LinqToDB.Mapping;
using System;

#pragma warning disable 1573, 1591
#nullable enable

namespace DataModel
{
	[Table("ReactionRole")]
	public class ReactionRole
	{
		[Column("Id"                   , IsPrimaryKey = true, IsIdentity = true, SkipOnInsert = true, SkipOnUpdate = true)] public int       Id                    { get; set; } // integer
		[Column("EmoteName"                                                                                              )] public string?   EmoteName             { get; set; } // text
		[Column("RoleId"                                                                                                 )] public ulong   RoleId                { get; set; } // numeric(20,0)
		[Column("ReactionRoleMessageId"                                                                                  )] public int       ReactionRoleMessageId { get; set; } // integer
		[Column("DateAdded"                                                                                              )] public DateTime? DateAdded             { get; set; } // timestamp (6) without time zone

		#region Associations
		/// <summary>
		/// FK_ReactionRole_ReactionRoleMessage_ReactionRoleMessageId
		/// </summary>
		[Association(CanBeNull = false, ThisKey = nameof(ReactionRoleMessageId), OtherKey = nameof(DataModel.ReactionRoleMessage.Id))]
		public ReactionRoleMessage ReactionRoleMessage { get; set; } = null!;
		#endregion
	}
}
